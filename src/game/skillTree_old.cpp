...
int height;
int *already_rendered_at_each_depth; //holds the number of nodes already render at each depth 
int *nodes_per_depth; //holds the total number of nodes at each depth
int * rendered_nodes;//holds the ids of the nodes that have been rendered already
...
//updates the number_of_nodes int with the number of nodes of tree
void NumberOfNodes(Node *tree, int * number_of_nodes){
	(*number_of_nodes)++;
	//iterate through tree
	for(size_t i = 0; i < tree->children_count; i++)
	{	
			NumberOfNodes(&tree->children[i], number_of_nodes);
	}
}

//returns the number of nodes of a tree at targetDepth. Call it with current_depth = 0
...
void renderNode(Node *node, int parent_x,int parent_y,int node_x,int node_y){
	...
	//if node has't been draw yet draw it
	if(true){
...
void renderTree(Node *tree, int current_depth,int parent_x, int parent_y){
	...
	int x_position = (already_rendered_at_each_depth[current_depth]+1)*node_offset;
	int y_position = (current_depth+1)*100;

	//render node and line 
	renderNode(tree,parent_x,parent_y,x_position,y_position);
	already_rendered_at_each_depth[current_depth]++; //update the number of nodes rendered at that depth
...
SkillTree::SkillTree() {
	...
	NumberOfNodes(example_tree, &number_of_nodes);
	height = treeHeight(example_tree);
	already_rendered_at_each_depth = (int *)malloc(sizeof(int)*(height+1));
	nodes_per_depth = (int *)malloc(sizeof(int)*(height+1));
	rendered_nodes = (int*)malloc(sizeof(int)*number_of_nodes);
...
void SkillTree::render() {
	...
	//INIT GLOBAL TRACKING VALUES NEEDED TO RENDER TREE
	//inits rendered_nodes
	for(size_t i = 0; i < number_of_nodes; i++)
	{
		rendered_nodes[i]=0;
	}
	//inits already_rendered_at_each_depth and nodes_per_depth
	for(size_t i = 0; i <= height; i++){
		already_rendered_at_each_depth[i]=0;
