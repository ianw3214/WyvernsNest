...
SettingsMenu::SettingsMenu() : 
	background("res/assets/menu/settingsmenu.png"),
...
void SettingsMenu::handleEvent(const SDL_Event &e) {
	if (e.type == SDL_KEYDOWN) {
		...
		if (e.key.keysym.sym == SDLK_DOWN) {
			selected_option++;
			if (selected_option >= NUM_BUTTONS_SET) selected_option = NUM_BUTTONS_SET - 1;
		}
...
void SettingsMenu::render() {
	...
	for (int i = 0; i < NUM_BUTTONS_SET; ++i) {
...
void SettingsMenu::switchToCurrentState() {
...
int SettingsMenu::getButtonIndexAtPos(ScreenCoord coord) {
	for (int i = 0; i < NUM_BUTTONS_SET; ++i) {
